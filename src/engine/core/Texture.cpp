...
void Texture::fromBuffer(Device* device, std::span<const std::byte> buffer, VkFormat format, std::uint32_t texWidth, std::uint32_t texHeight)
{
    ...
    // NOTE: Step 2 - copy data to a staging buffer
    VkBuffer imgStagingBuffer{VK_NULL_HANDLE};
    VmaAllocation imgStagingAllocation{VK_NULL_HANDLE};
    const VkBufferCreateInfo imgStagingBufferCI{
        .sType = VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO,
    ...
    std::memcpy(imgStagingAllocInfo.pMappedData, buffer.data(), buffer.size());

    // NOTE: Step 3 - prepare the image transitions
    ...
    const VmaAllocationCreateInfo texImageAllocCI{.usage = VMA_MEMORY_USAGE_AUTO};
    chk(vmaCreateImage(m_device->allocator(), &imageCI, &texImageAllocCI, &m_image, &m_allocation, nullptr));
    ...
    // NOTE: Step 4 - record image transition command buffer
    VkCommandBuffer oneTimeCmdBuffer{ m_device->createCommandBuffer() };
    ...
    // NOTE: Step 5 - finish command buffer recording and submit to queue
    m_device->submitCommandBuffer(oneTimeCmdBuffer);

    vmaDestroyBuffer(m_device->allocator(), imgStagingBuffer, imgStagingAllocation);

    // NOTE: Step 6 - create texture resources
    const VkSamplerCreateInfo samplerCI{
        .sType = VK_STRUCTURE_TYPE_SAMPLER_CREATE_INFO,
        .magFilter = VK_FILTER_LINEAR,
        .minFilter = VK_FILTER_LINEAR,
