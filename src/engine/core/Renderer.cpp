...
Renderer::Renderer(
    ...
    : m_device{device}, m_window{window}, m_swapchain{swapchain}, m_pipeline{pipeline}, m_textures(std::move(textures))
{
...
void Renderer::recordCommandBuffers(const ObjModel& mesh)
{
    ...
    const VkDependencyInfo barrierDependencyInfo{
        ...
        .imageMemoryBarrierCount = static_cast<std::uint32_t>(outputBarriers.size()),
    ...
    const VkDependencyInfo barrierPresentDependencyInfo{
        .sType = VK_STRUCTURE_TYPE_DEPENDENCY_INFO,
