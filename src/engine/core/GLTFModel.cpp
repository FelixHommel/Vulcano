...
GLTFModel::GLTFModel(Device& device) : m_device{ device } {}
...
void GLTFModel::loadImages(tinygltf::Model& input)
{
    ...
    for(std::size_t i{ 0 }; i < m_images.size(); ++i)
    {
        ...
        {
            const auto pixelCount{ static_cast<std::size_t>(glTFImage.width)
                                   * static_cast<std::size_t>(glTFImage.height) };
            ...
            m_images[i].texture = std::move(
                texture::fromBuffer(
                    &m_device,
                    std::as_bytes(std::span{ rgba }),
            ...
            m_images[i].texture = std::move(
                texture::fromBuffer(
                    &m_device,
                    std::as_bytes(std::span{ glTFImage.image }),
                    VK_FORMAT_R8G8B8A8_UNORM,
                    glTFImage.width,
                    glTFImage.height
                )
...
void GLTFModel::loadMaterials(tinygltf::Model& input)
{
    ...
    for(std::size_t i{ 0 }; i < m_materials.size(); ++i)
    {
        ...
        if(glTFMaterial.values.contains(::BASE_COLOR_FACTOR))
            m_materials[i].baseColorFactor
                = glm::make_vec4(glTFMaterial.values[::BASE_COLOR_FACTOR].ColorFactor().data());
    ...
    if(inputNode.mesh > -1)
    {
        ...
        for(std::size_t i{ 0 }; i < mesh.primitives.size(); ++i)
        {
...

            {
                ...
                {
                    const auto& accessor{
                        input.accessors[glTFPrimitive.attributes.find(::GLTF_PRIMITIVE_ATTRIBUTE_POSITION)->second]
                    };
                ...
                {
                    const auto& accessor{
                        input.accessors[glTFPrimitive.attributes.find(::GLTF_PRIMITIVE_ATTRIBUTE_NORMAL)->second]
                    };
                    const auto& view{ input.bufferViews[accessor.bufferView] };
                ...
                {
                    const auto& accessor{
                        input.accessors[glTFPrimitive.attributes.find(::GLTF_PRIMITIVE_ATTRIBUTE_TEX_COORD)->second]
                    };
                    const auto& view{ input.bufferViews[accessor.bufferView] };
                ...
                for(std::size_t v{ 0 }; v < vertexCount; ++v)
                {
                    vertexBuffer.emplace_back(
                        ...
                        glm::normalize(
                            glm::vec3(normalsBuffer ? glm::make_vec3(&normalsBuffer[v * 3]) : glm::vec3(0.f))
                        ),
            ...
            }
            {
                ...
                case TINYGLTF_PARAMETER_TYPE_UNSIGNED_INT:
                    {
                        const auto* buf{ reinterpret_cast<const std::uint32_t*>(
                ...
                case TINYGLTF_PARAMETER_TYPE_UNSIGNED_SHORT:
                    {
                        const auto* buf{ reinterpret_cast<const std::uint16_t*>(
                            &buffer.data[accessor.byteOffset + bufferView.byteOffset]
                        ) };
        ...
        for(const auto& primitive : node.mesh.primitives)
        {
            if(primitive.indexCount > 0)
            {
                ...
                vkCmdBindDescriptorSets(
                    ...
                    1,
                    1,
                    &m_images[texture.imageIndex].descriptorSet,
                    0,
                    nullptr
                );
